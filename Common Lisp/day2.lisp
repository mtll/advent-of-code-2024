...
(defun safe-p (levels &optional (tolerance 0))
  (labels ((unsafe (p n tol)
             (cond ((null n) nil)
                   ((or (null p) (<= 1 (- (car n) (car p)) 3))
                    (unsafe (cons (car n) p) (cdr n) tol))
                   ((> tol 0)
                    (and (unsafe p (cdr n) (1- tol))
                         (unsafe (cdr p) n (1- tol))))
                   (t t))))
    (or (not (unsafe nil levels tolerance))
        (not (unsafe nil (reverse levels) tolerance)))))
